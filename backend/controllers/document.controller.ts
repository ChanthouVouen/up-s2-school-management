...
function resolveUploadedFile(fileUrl: string): string | null {
  const uploadPrefix = "/uploads/";
  if (!fileUrl.startsWith(uploadPrefix)) return null;
...
export const getDocuments: RequestHandler = asyncHandler(async (req, res) => {
  ...
  if (
    status &&
    Object.values(DocumentStatus).includes(String(status) as DocumentStatus)
  )
    where.status = status;
  if (
    type &&
    Object.values(DocumentType).includes(String(type) as DocumentType)
  )
    where.type = type;
...
export const getDocumentById: RequestHandler = asyncHandler(
  async (req, res) => {
    ...
    const document = await prisma.document.findUnique({
      ...
      include: {
        ...documentInclude,
        reviews: { orderBy: { uploadedAt: "desc" } },
      },
    });
    if (!document) {
...
export const reviewDocument: RequestHandler = asyncHandler(async (req, res) => {
  ...
  if (status !== "VERIFIED" && status !== "REJECTED") {
    res
      .status(400)
      .json({ message: "Review status must be VERIFIED or REJECTED" });
    return;
  }
...
export const deleteDocument: RequestHandler = asyncHandler(async (req, res) => {
  ...
  const filePath = resolveUploadedFile(document.fileUrl);
  if (filePath) {
...
export const getDocumentReviews: RequestHandler = asyncHandler(
  async (req, res) => {
    const id = parseId(String(req.params.id));
    if (!id) {
      res.status(400).json({ message: "Invalid document ID" });
