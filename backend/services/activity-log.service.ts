...
export async function getActivityLogsService(query: Record<string, any>) {
  const { search, type, from, to, page = '1', limit = '10' } = query;
...
export async function getActivityLogStatsService() {
  ...
  return { total, totalToday, byType };
}
